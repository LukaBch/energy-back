...
def compute_hour_consumptions(selected_appliances, max_energy_consumption):
    ...
    for i in APPLIANCES.keys():
        coefficients[i] = round(APPLIANCES[i]["power"]/1000, 2) if i in selected_appliances else 0
    ...
    lp_problem += (coefficients[1] * x_1 + coefficients[2] * x_2 + coefficients[3] * x_3 + coefficients[4] * x_4 + coefficients[5] * x_5 + coefficients[6] * x_6 + coefficients[7] * x_7 + coefficients[8] * x_8)

    # Add the constraints
    lp_problem += (coefficients[1] * x_1 + coefficients[2] * x_2 + coefficients[3] * x_3 + coefficients[4] * x_4 + coefficients[5] * x_5 + coefficients[6] * x_6 + coefficients[7] * x_7 + coefficients[8] * x_8 <= max_energy_consumption)
    if coefficients[1] == 0:
        lp_problem += x_1 == 0
    if coefficients[2] == 0:
        lp_problem += x_2 == 0
    ...
    if (coefficients[1] > 0 or coefficients[2] > 0):
        lp_problem += (HOURS_CATEGORIES["F"]["min"] <= x_1 + x_2)
        lp_problem += (x_1 + x_2 <= HOURS_CATEGORIES["F"]["max"])
    
    if (coefficients[3] > 0 or coefficients[4] > 0 or coefficients[5] > 0):
        lp_problem += (HOURS_CATEGORIES["A"]["min"] <= x_3 + x_4 + x_5)
        lp_problem += (x_3 + x_4 + x_5 <= HOURS_CATEGORIES["A"]["max"])
    
    if (coefficients[6] > 0 or coefficients[7] > 0 or coefficients[8] > 0):
        lp_problem += (HOURS_CATEGORIES["L"]["min"] <= x_6 + x_7 + x_8)
        lp_problem += (x_6 + x_7 + x_8 <= HOURS_CATEGORIES["L"]["max"])
